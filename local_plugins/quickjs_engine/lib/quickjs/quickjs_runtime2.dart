...
class QuickJsRuntime2 extends JavascriptRuntime {
        ...
        switch (type) {
          case JSChannelType.METHON:
           ...
           return _dartToJs(
               ...
               func.invoke(
                 pargs,
                 null, // Skip 'this' conversion: enumerating global object properties causes thousands of re-entrant FFI calls → SIGSEGV
               ));
  ...
  void initChannelFunctions() {
    ...
    (setToGlobalObject as JSInvokable).invoke([
      'sendMessage',
      (dynamic channelName, dynamic message) {
        final channelFunctions = JavascriptRuntime
            .channelFunctionsRegistered[getEngineInstanceId()]!;

        final name = channelName?.toString() ?? '';
        if (channelFunctions.containsKey(name)) {
          // 防御性解码：message 可能是已解码的 Map（FFI 直接传递）或 JSON String
          final args = message is String
              ? () {
                  try {
              ...
              : message;
          return channelFunctions[name]!.call(args);
        } else {
          print('No channel $name registered');
        }
        if (JavascriptRuntime.debugEnabled) {
          print('CHANNEL: $name - Message: $message');
        }
      }
    ]);
  }
