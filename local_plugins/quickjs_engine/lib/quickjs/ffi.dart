...
//     pub-cache install or a path-dep — Dart resolves `package:quickjs_engine`
...
DynamicLibrary _openTestLib() {
  ...
  final candidates = <String>[
    'packages/quickjs_engine/native/build/$libName',
    '../packages/quickjs_engine/native/build/$libName',
...
// Lazy lookup: jsSetMemoryLimit may not exist in older DLL/.so builds.
// If the symbol is missing, we simply skip setting a memory limit.
void Function(Pointer<JSRuntime>, int)? _jsSetMemoryLimitFn;
bool _jsSetMemoryLimitChecked = false;
void jsSetMemoryLimit(Pointer<JSRuntime> rt, int limit) {
  if (!_jsSetMemoryLimitChecked) {
    _jsSetMemoryLimitChecked = true;
    try {
      _jsSetMemoryLimitFn = _qjsLib
          .lookup<
              NativeFunction<
                  Void Function(
                    Pointer<JSRuntime>,
      ...
      // Symbol not found in this build - memory limit will be skipped.
    }
  }
  _jsSetMemoryLimitFn?.call(rt, limit);
}
