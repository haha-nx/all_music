...
import 'package:quickjs_engine/javascriptcore/jscore_runtime.dart';
...
//import 'package:quickjs_engine/quickjs-sync-server/quickjs_oasis_jsbridge.dart';
//import 'package:quickjs_engine/quickjs/quickjs_runtime.dart';
...
JavascriptRuntime getJavascriptRuntime({
  ...
  // 默认 8MB 栈 + 64MB 内存 — 洛雪系音源脚本采用高度混淆的自解密 VM，
  // 递归深度远超普通脚本，QuickJS-NG 默认 1MB 栈必然溢出导致 inited 事件无法触发。
  //
  // 注意：native 层 jsSetMaxStackSize 会把此值钳制为「实际线程栈 − 1MB」，
  // 避免 stack_limit 落到线程 guard page 之下导致 js_check_stack_overflow
  // 失效、深度递归打穿原生栈引发 SIGSEGV（Android 主线程栈通常仅数 MB）。
  // 钳制后，超深递归会抛出可被 catch 的 StackOverflow JS 异常而非闪退。
  final int stackSize = extraArgs?['stackSize'] ?? 8 * 1024 * 1024;
  final int memoryLimit = extraArgs?['memoryLimit'] ?? 64 * 1024 * 1024;
...
MethodChannel _methodChannel = const MethodChannel('io.quickjs_engine')
